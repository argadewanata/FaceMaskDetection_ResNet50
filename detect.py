...
def detect_and_predict_mask(frame, model, face_detection, draw_landmarks=True):
    # Convert the frame from BGR (default by OpenCV) to RGB (required by Mediapipe)
    ...
    # Return the frame with bounding boxes and predictions drawn on it
    return frame
    ...
    with mp_face_detection.FaceDetection(model_selection=0, min_detection_confidence=0.5) as face_detection:
        while True:
            ...
            if key & 0xFF == ord('q'):
                ...
                print(
                    f"Draw face landmarks toggled to {draw_face_landmarks}")
